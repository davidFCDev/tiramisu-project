...
document.addEventListener("DOMContentLoaded", function () {
  ...
  actionButtons.forEach(function (button) {
    button.addEventListener("click", function () {
      ...
      if (isModalVisible) {
        confirmationModal.style.display = "none";
        // Cambiar el color del ícono a blanco cuando el menú se cierra
        button.querySelector("img").src = "/images/Menu.svg"; // Ruta del ícono blanco
      } else {
        confirmationModal.style.display = "flex";
        // Cambiar el color del ícono a azul cuando el menú se abre
        button.querySelector("img").src = "/images/BlueMenu.svg"; // Ruta del ícono azul
      }
    });
  });
});
